    ...
    >
      <div className="flex flex-col md:flex-row min-h-[340px]">
        ...
        <div className="flex-1 p-8 md:p-10 flex flex-col justify-center">
          ...
          {item.restaurantName && (
            <p className="text-base font-semibold text-brand mb-4 cursor-pointer hover:text-brand-hover transition-colors" onClick={handleNavigate}>
              {item.restaurantName}
            </p>
          )}
        ...
        <div className="md:w-[380px] h-[280px] md:h-auto relative p-4">
          ...
          <div className="absolute top-6 right-6 flex gap-2">
            ...
            >
              <Bookmark className={`w-5 h-5 ${isInColl ? 'text-yellow-500 fill-current' : 'text-brand'}`} />
            </button>
            <button
              onClick={toggleFav}
      ...
      <div className="relative h-[180px] overflow-hidden">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2">
          ...
          <button
            ...
            title={isInColl ? "Đã có trong bộ sưu tập" : "Thêm vào bộ sưu tập"}
          >
            <Bookmark className={`w-4 h-4 ${isInColl ? 'text-yellow-500 fill-current' : 'text-brand'}`} />
          </button>
        </div>
...
function FeatureBar() {
  const features = [
    { icon: Sparkles, bg: 'bg-brand-muted dark:bg-brand/20', color: 'text-brand', title: 'AI-Powered Recommendations', desc: 'Personalized just for you' },
