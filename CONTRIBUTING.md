>[!warning] Luôn pull trước mỗi phiên làm việc và khi hoàn thành (push)
...
> Giữ nguyên nhánh sau khi hoàn thành, thêm `done` sau tên nhánh
...
Viết commit ngắn gọn, rõ ràng bằng tiếng Anh.
...
- **Bước 1:** `git pull origin main` (Cập nhật code mới nhất từ nhánh chính).
...
- **Bước 6:** Lên GitHub tạo Pull Request (PR) gộp nhánh của bạn vào `main`.

#### 4. Quy tắc Review & Hợp nhất (Merge Rules)
- Phải có ít nhất 1 thành viên khác trong nhóm xem qua code (Review) và bấm "Approve" trên GitHub thì mới được Merge vào `main`.
